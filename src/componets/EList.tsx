...
const EList: React.FC = () => {
  ...
  return <div style={{ width: '100%', height: 500 , overflow: 'auto', position: 'relative' }} ref={ref}>
   <p style={{width:'200%'}}>
     ...
     Scrollbars are an essential component of graphical user interfaces (GUIs), enabling users to navigate and manipulate content that exceeds the visible area within a limited space. Scrollbars are commonly found in windows, dialog boxes, text editors, web browsers, and other applications, providing users with a convenient way to navigate and view content.

     #### Components of a Scrollbar

     1. **Track**: The background part of the scrollbar, typically a vertical or horizontal bar.
     2. **Thumb**: The movable small block within the track, indicating the current position of the displayed content within the entire content.
     3. **Arrow Buttons**: Buttons located at both ends of the scrollbar that allow users to fine-tune the display position of the content by clicking them.
     ...
     - **Positioning**: The position of the thumb reflects the relative position of the currently displayed content within the entire content.
     - **Zooming**: In some applications, scrollbars can also be used to adjust the zoom level of the content.

     #### Types of Scrollbars

     1. **Vertical Scrollbar**: Typically located on the right side of a window or text box, used for scrolling content up and down.
     2. **Horizontal Scrollbar**: Usually located at the bottom of a window or text box, used for scrolling content left and right.
     3. **Combined Scrollbars**: In some applications, both vertical and horizontal scrollbars appear simultaneously to support all-directional content navigation.

     #### Design and User Experience of Scrollbars

     The design of scrollbars directly impacts user experience. A well-designed scrollbar should possess the following characteristics:

     - **Visibility**: Scrollbars should be clearly visible when needed but should not interfere with the user when not in use.
     - **Usability**: The thumb should be easy to drag, and the arrow buttons should respond quickly.
     - **Consistency**: The appearance and behavior of scrollbars should be consistent across different applications and operating systems.

     #### Technical Implementation of Scrollbars

     In web design, scrollbars can be customized using CSS to match the overall style of the webpage. In desktop applications, scrollbars are typically provided by the operating system, but developers can also customize them through programming interfaces.

     #### Conclusion

     Scrollbars, as a fundamental element of the user interface, offer users an efficient way to navigate content. Whether browsing the web, editing documents, or operating applications, scrollbars are crucial tools for enhancing user experience. With technological advancements, the design and functionality of scrollbars continue to evolve to meet users' demands for more efficient and intuitive operations.
     ### Introduction to Scrollbars
